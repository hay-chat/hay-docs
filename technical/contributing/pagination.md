...
**Search Fields:** `["name", "description"]` (default); `"prompt_template"` also selectable
...
> ⚠️ **Not yet wired up:** This schema exists in `entity-list-inputs.ts` (`messageListInputSchema` / `messageFiltersSchema`), but no route currently uses it. Treat the above as a planned/target shape, not current behavior.
