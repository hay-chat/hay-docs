...
2. For each subdirectory, `registerPlugin(pluginPath, sourceType, organizationId)`:
   ...
   - Plugin ID = `package.json.name`. Display name = `hay-plugin.displayName` or a Title-Cased version of the package name with the `hay-plugin-` prefix stripped (`parseDisplayName`).
   - Build a manifest from the `hay-plugin` block and persist it as the `manifest` jsonb column. The primary `type` comes from `hay-plugin.category`, plus extra types inferred from capabilities (`inferTypeFromCapabilities`: `mcp` → `mcp-connector`, `routes`/`messages` → `channel`, `sources` → `retriever`, `products` → `products`; if nothing matches, the inferred list defaults to `["mcp-connector"]`) and `documentImporter: true` → `document_importer`. The `hay-plugin.env` allow-list is stored on the manifest as `permissions.env`, and `capabilities` is mirrored into `permissions.api`.
   - Load i18n JSON files from the plugin's `i18n/` directory into `manifest.i18n`.
   - Calculate a SHA-256 checksum over the plugin's `.ts`/`.js` files (excluding `node_modules` and `dist`) for change detection (`calculatePluginChecksum`).
...
interface HayPluginBlock {
  ...
  env?: string[];               // allow-list of host env vars the worker may inherit → manifest.permissions.env
  auth?: ...;
...
2. If `authMethod === "oauth"` and the stored token expires within 5 minutes, refresh it via `oauthService.refreshToken()` **before** spawning.
3. Allocate a port (`server/services/port-allocator.service.ts`) and resolve the org's config with `resolveConfigForWorker` (`server/lib/config-resolver.ts`), which merges DB config, `env:` fallbacks, and auth credentials. The config schema used for env fallbacks comes from the cached worker metadata (`plugin_registry.metadata.configSchema`).
...
5. Environment injection (`buildSDKEnv`):
   ...
   - Host env vars from the plugin's allow-list (`manifest.permissions.env`, i.e. `hay-plugin.env`) — filtered through a deny-pattern list (`SECRET`, `PASSWORD`, `TOKEN`, `PRIVATE_KEY`, `CREDENTIAL`, `^DB_`, `^DATABASE`, `^REDIS`, `^AWS_`, `^OPENAI`, `^ANTHROPIC`, `^JWT`, `^SMTP`, `^GITHUB`, `^NPM`, `^PLUGIN_ENCRYPTION`, `^HAY_`) so secret-looking names are blocked by default
...
`POST /disable` on the worker (5s timeout, best-effort), then `SIGTERM`, then `SIGKILL` after 5 seconds if it hasn't exited. The port is released, the worker removed from the map, and the instance row updated (`runtimeState = "stopped"`, `running = false`); the process `exit` handler also updates state/health when a worker dies on its own.
...
| `metadata`, `metadataFetchedAt`, `metadataState`                                                       | jsonb / timestamptz / varchar | plugin-global cache of the worker's `/metadata` response; `metadataState` defaults to `"missing"` |
...
| `onEnable`       | **Never.**                                                                         | Typed in the SDK (`types/plugin.ts`, marked "CORE-ONLY, not called by runner") — but nothing in core calls it either. Reserved for a future core-side path; do not build on it.                   |
...
Some providers (e.g. Meta) deliver all events for all customers to **one** webhook URL that carries no org identifier. A plugin can declare a `WebhookRoutingDescriptor` via `register.webhookRouting` (`packages/plugin-sdk/types/webhook-routing.ts`) — pure data, no code: an HMAC-SHA256 signature spec (`header`, `format: "sha256-hmac"`, secret source), an optional GET verification-challenge spec, and dot-path extraction rules (`routeKeyPath: { itemsPath, keyPath }`).

The dispatch point is in the proxy (`server/routes/v1/plugins/proxy.ts`): a request to `.../webhook` with **no** org identifier, for a plugin that declared a routing descriptor, is diverted to `webhookRouterService.handle()` (`server/services/webhook-router.service.ts`), which:

1. Answers the provider's GET verification handshake if declared (echoes the challenge param after checking the verify token).
2. For POSTs, verifies the HMAC over the raw body bytes (`verifyHmacSha256` from `server/services/plugin-route.service.ts`).
3. Extracts a routing key per payload entry (`routeKeyPath`), resolves key → org via `plugin_webhook_routes`, and fans each entry out to the right org's worker `POST /webhook` (with credential-bearing headers stripped).
...
Webhooks that do identify the org go straight through the proxy to the worker's registered `/webhook` route. The plugin verifies/filters the event and then calls **back into core** with `messages.receive` (see next section). Inbound dedupe is core-side: if the plugin passes `metadata.mid`, core claims it atomically via Redis `SET ... NX EX` and drops duplicates (`server/routes/v1/plugin-api/trpc.ts`); channels that don't pass a `mid` get no dedupe.
...
- `conversation_status_changed` → `pending-human` → `POST /escalate` on the worker (web channel skipped). This is best-effort: a 404 from plugins that don't implement `/escalate` is silently tolerated.
...
Workers call back into core over tRPC-HTTP using the `HAY_API_URL`/`HAY_API_TOKEN` env vars. Every procedure is a `pluginProcedure` gated by the worker's JWT (org + plugin + capabilities; `server/trpc/middleware/plugin-auth.ts`). Current surface:
...
- `products.upsertMany`, `products.delete` (source is the authenticated plugin ID — a plugin can only touch its own products)
...
- **`onEnable` is dead code** — typed in the SDK but invoked by neither the runner nor core.
...
- **`PluginApiClient` is duplicated per channel plugin** (`plugins/core/instagram/src/plugin-api.ts`, and likewise in `chatwoot`, `email`, `whatsapp`) instead of being an SDK export.
