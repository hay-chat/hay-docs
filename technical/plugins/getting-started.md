...
- **`onValidateAuth`** is invoked by core when the user saves credentials. Do a real API round-trip; a thrown `Error` message is shown to the user. If you don't implement it, credentials are assumed valid.
...
3. Enable it, then open its settings page (`/integrations/plugins/hay-plugin-nasa`) and enter `DEMO_KEY` as the API key. Saving triggers `onValidateAuth` — an invalid key surfaces your thrown error message.
